...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox) {
        ...
        const role = await this._askUserForRoleAsync(roles, profile.azure_default_role_arn);
        await this._assumeRoleAsync(profileName, samlResponse, role);
    },
    ...
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, defaultUsername) {
        ...
        try {
            ...
            // FOr now disable the interceptor so things work
     ...
     * @returns {Promise.<string>} The selected role
     * @private
     */
    async _askUserForRoleAsync(roles, defaultRoleArn) {
        let role;
        if (roles.length === 0) {
